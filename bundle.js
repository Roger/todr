...
/***/ function(module, exports, __webpack_require__) {
	...
	var mui = __webpack_require__(24);
	var AppCanvas = React.createFactory(mui.AppCanvas);
	var AppBar = React.createFactory(mui.AppBar);

	var List = React.createFactory(__webpack_require__(10).List);
	var History = React.createFactory(__webpack_require__(11));

	// Needed for onTouchTap Can go away when react 1.0 release
	// Check this repo: https://github.com/zilverline/react-tap-event-plugin
	var injectTapEventPlugin = __webpack_require__(13);
	injectTapEventPlugin();

	var stores = __webpack_require__(12);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var DragDropMixin = __webpack_require__(8).DragDropMixin;
	...
	var ItemsStore = Reflux.createStore({
	  ...
	  updateDoc: function(doc) {
	    ...
	    binding.
	      ...
	      update(meta, 'fromCouch', function() {
	        return true;
	      }).commit();
	  },
	  listenBindings: function() {
	    this.sortListenID = this.sortingBinding.addListener(function (changes) {
	      ...
	      if(this.sortingBinding.meta().get("fromCouch")) {
	        return;
	      }
	      ...
	      this.timeouts[docID] = setTimeout(function() {
	        this.db.put(this.sortingBinding.toJS());
	        delete(this.timeouts[docID]);
	      }.bind(this), 1000);
	    }.bind(this));
	    ...
	    this.itemsListenID = this.itemsBinding.addListener(function (changes) {
	      ...
	      var itemBind = this.itemsBinding.sub(docID);
	      // Don't try to update pouch docs from pouch
	      if(itemBind.meta().get("fromCouch")) {
	        return;
	      }

	      var item = itemBind.get();
	  ...
	  listenCouch: function() {
	    ...
	    // var sortingBinding = this.sortingBinding;
	  ...
	  onAdd: function (title, last) {
	    ...
	    var meta = itemBinding.meta();
	    itemBinding.
	      atomically().
	      set(Immutable.Map({_id: id, doc_type: "item", title: title})).
	      update(meta, 'fromCouch', function() {
	        return false;
	      }).commit();
	  ...
	  updateSorting: function(id, selectedIdx){
	    ...
	    var meta = this.sortingBinding.meta();
	    this.sortingBinding.
	      atomically().
	      update(meta, 'fromCouch', function() {
	        return false;
